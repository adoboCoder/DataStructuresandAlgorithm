...
class LCM_1509MinimumDifferenceBetweenLargestAndSmallestValueInThreeMoves {
    ...
    if(nums.length < 5) {
        ...
        for(int num : nums) {
            minHeap.add(num);
            maxHeap.add(num);
            if(minHeap.size() > 4 || maxHeap.size() > 4) {
                minHeap.poll();
                maxHeap.poll();
            }
        }
    ...
    public static void main(String[] args) {
        System.out.println(LCM_1509MinimumDifferenceBetweenLargestAndSmallestValueInThreeMoves.minDifference(new int[]{1,5,0,10,14}));
